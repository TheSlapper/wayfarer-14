using Content.Server.Chat.Systems; // Wayfarer: Goob's shared-chat refactor is absent in Wayfarer, so this system moved to Server for IC speech
using Content.Shared._Goobstation.SlotMachine.CoinFlipper;
...
namespace Content.Server._Goobstation.SlotMachine.CoinFlipper;
...
public sealed class CoinFlipperMachineSystem : EntitySystem
{
    ...
    [Dependency] private readonly ChatSystem _chatSystem = default!; // Wayfarer: SharedChatSystem<ChatSystem
    ...
    private void OnInteractHandEvent(EntityUid uid, CoinFliperComponent comp, ActivateInWorldEvent args)
    {
        ...
        var doAfter =
         new DoAfterArgs(EntityManager, uid, comp.DoAfterTime, new CoinFlipperDoAfterEvent(), uid)
         {
             BreakOnMove = false,
             BreakOnDamage = false,
             // Wayfarer: Removed MultiplyDelay = false, field absent in Wayfarer's DoAfterArgs
         };
    ...
    private void OnSlotMachineDoAfter(EntityUid uid, CoinFliperComponent comp, CoinFlipperDoAfterEvent args)
    {
        ...
        // Wayfarer: Lose buzz removed, the machine speaks the loss like it does its wins
        //_audio.PlayPredicted(comp.LoseSound, uid, args.User); // If nothing then lose
        _chatSystem.TrySendInGameICMessage(uid, Loc.GetString("coinflipper-lose"), InGameICChatType.Speak, hideChat: false, hideLog: true, checkRadioPrefix: false);
    }
}
